...
#additional package to calculate statistical significance
#install.packages("ggpubr")
>>>>>>> 89e9dc78d16d8ed1b3cdc960dc956b506b9398f0
library(ggpubr)
...
```{r Section 6.3}
ggplot(fish_counts, aes(x = hour, y = proportions, color = genotype)) + annotate("rect", xmin = 0, xmax = 9, ymin = 0, ymax = Inf, fill = "grey", alpha = 0.6) + annotate("rect", xmin = 23, xmax = 24, ymin = 0, ymax = Inf, fill = "grey", alpha = 0.6) + geom_point() + geom_line() + facet_wrap(~state) + ggtitle("Plot 7: Sleep and wake proportions in wildtype vs knockouts")
...
```{r Section 6.4}
fish_counts %>% filter(hour %in% 9:23, genotype %in% c("WT", "Hom"))  %>% ggplot(., aes(x = state, y = n, color = genotype)) + geom_boxplot() + scale_colour_manual(values = c("#F8766D" , "#619CFF")) + stat_compare_means(method = "t.test") + ggtitle("Plot 8: Sleep and wake proportions in wildtype and knockouts during the day") 
...
```{r Section 6.5}
fish_counts %>% filter(hour %in% 0:8, genotype %in% c("WT", "Hom"))  %>% ggplot(., aes(x = state, y = n, color = genotype)) + geom_boxplot() + scale_colour_manual(values = c("#F8766D" , "#619CFF")) + stat_compare_means(method = "t.test")  + ggtitle("Plot 9: Sleep and wake proportions in wildtype and knockouts during the night")
```
...
There is no obvious difference in sleep consolidation. Knockouts do not have more fragmented sleep compared to wildtype.
