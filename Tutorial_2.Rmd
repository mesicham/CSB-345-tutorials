...
ggplot(fish_WT, aes(x = time, y = Dist_6s, colour = daytime)) + geom_line()
#plot all observations (264k), rounded to the nearest hour
ggplot(fish_WT, aes(x = round_date(time, unit = "minute"), y = Dist_6s, colour = daytime)) + geom_line() 
```

```{r timecourse}
#plot all observations, rounded to the nearest hour for one day
fish_WT %>% filter(day == 10) %>% separate(., col = time, into = c("date", "time"), sep = " ") 
fish_WT[is.na(fish_WT$time)]
ggplot(., aes(x = time, y = Dist_6s, colour = daytime)) + geom_line() + scale_x_datetime(breaks=date_breaks("3 hours"))
...
```{r}
ggplot(fish_counts, aes(x = state, y = n)) + geom_boxplot() + facet_wrap(~hour)
